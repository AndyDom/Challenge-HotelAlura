...
public class ReservasView extends JFrame {
	...
	public ReservasView() {
		...
		txtFechaS.addPropertyChangeListener(new PropertyChangeListener() {
			...
			public void propertyChange(PropertyChangeEvent evt) {
				calcularTarifa(txtFechaE,txtFechaS);
			}
		});
	...
	private void calcularTarifa(JDateChooser fechaE, JDateChooser fechaS) {
		if (fechaE.getDate()!= null && fechaS.getDate() != null) {
			Calendar fechaEntrada = fechaE.getCalendar();
			Calendar fechaSalida = fechaS.getCalendar();
			
			if (fechaEntrada.compareTo(fechaSalida)<=0) {
				int dias = -1;
				int tarifaDiaria = 200;
				int tarifaTotal;
				
				while(fechaEntrada.before(fechaSalida)||fechaEntrada.equals(fechaSalida)) {
					dias++;
					fechaEntrada.add(Calendar.DATE,1);
				}
				
				tarifaTotal = dias * tarifaDiaria;
				txtValor.setText(""+ tarifaTotal);
			}else {
				JOptionPane.showMessageDialog(this, "La fecha de ingreso no puede ser mayor a la de egreso.");
			}
		}
	}
